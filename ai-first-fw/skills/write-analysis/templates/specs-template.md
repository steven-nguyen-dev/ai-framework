...
carries conclusions at the level a reader settles scope from, it draws each flow as a sequence, it
...
- Start every section 3 item with a verb — Add, Remove, Send, Replace, Implement, Audit, Keep,
  Reject, Read — and name the system it lands in.
- Group section 3 by the team that builds it, then by endpoint, flow or domain inside each team,
...
- Put an action in 3.1 or 3.2, and a fact in 3.3, so each thing appears once.
...
**The flow diagrams.** Section 2 carries one Mermaid `sequenceDiagram` per flow in mapping section
...
### 3.1 [THIS_CODEBASE] work — `integration`

*(What this team builds. Start each item with a verb. Where a value crosses an `internal` contract,
this side widens its ingress and becomes reachable here first; 3.2 carries the request that follows.)*
...
### 3.2 [OTHER_SYSTEM] changes to request — `internal`

*(What another internal team builds. Give each item a priority, and name the `C-n` in 3.1 that makes
...
### 3.3 Facts that set the shape of the work
...
| D-6 | Every flow in section 2 runs end to end against the mocks. | integration | settled | `[path/to/mock/suite]` |
...
`internal` gap belongs in section 3 as work, and an unsettled question that does not block the start
