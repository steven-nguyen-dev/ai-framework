...
**Document Identifier:** `[KEY]-[TOPIC]-mapping-spec.md`
...
- Group by endpoint. One `###` section per endpoint that carries data, in the order the flow calls
  ...
  data comes from, `target` reaches the system it lands in. A property row sits under the endpoint
  that carries it.
...
- **Reason** is one clause naming why the target property needs that value. Keep it under about
  twelve words. "Enum lookup, see section 5" is a transformation; "the target routes work on this
  status" is a reason.
...
- Keep the sections this requirement uses and delete the rest.
...
section 4. Each flow here is the unit the mock run in the summary's definition of done exercises.
...
| 3 | `$.status_code` | `status` | `string` | Enum lookup, section 5 | The target routes work on this state | No | `"ACTIVE"` | `L-3` |
...
| 5 | `[Context]` | `[scope_key]` | `string` | Inject scope identifier | Scopes the record to one account | No | `"SCOPE_0001"` | `L-5` |
...
| 7 | `$.line_items[*].quantity` | `qty` | `integer` | Parse integer, default 0 | The target deducts stock on this number | No | `5` | `L-7` |
...
| # | Source field path | Target property | Type | Transformation | Reason | Nullable | Example | Claim |
| :-: | :--- | :--- | :--- | :--- | :--- | :-: | :--- | :--- |
| 1 | `$.attributes.[key_a]` | `field_code: "[key_a]"` | `singleSelect` | Map allowed values to `field_values[]` | The target renders a dropdown from these | Yes | `"red"` to `{"name": "Red", "value": "red"}` | `L-8` |
| 2 | `$.attributes.[key_b]` | `field_code: "[key_b]"` | `textField` | Flatten object, append unit | The target holds no numeric-with-unit type | Yes | `{"w": 10, "u": "cm"}` to `"10 cm"` | `L-9` |
