...
2. **Where** — `path:line` on the `+` side, read from the pinned SHA, then the quoted line. A line you
   cannot pin says so in place of the number.
...
read, else at the repo root. Name that path in the report.
