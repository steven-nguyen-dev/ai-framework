...
```
ai-first-fw/local-mcps/kibana/
├── .venv/             # Dedicated Python virtual environment (created by setup.sh)
...
```bash
ai-first-fw/local-mcps/kibana/.venv/bin/python3 \
  ai-first-fw/local-mcps/kibana/server.py --selftest
```
...
Credentials live in `.env` beside `server.py` (git-ignored; `.env.sample` is the template).
...
Registered for this repository in `.mcp.json` at the repo root. Pointing the client at the
virtualenv's interpreter means there is no venv to activate.

```json
{
  "mcpServers": {
    "kibana": {
      "command": "/Users/nguyennguyen.anchanto/Projects/ai-framework/ai-first-fw/local-mcps/kibana/.venv/bin/python3",
      "args": [
        "/Users/nguyennguyen.anchanto/Projects/ai-framework/ai-first-fw/local-mcps/kibana/server.py"
      ]
    }
  }
}
...
```bash
V=ai-first-fw/local-mcps/kibana/.venv/bin/python3
$V ai-first-fw/local-mcps/kibana/test_kql.py     # 34 tests — KQL translation
$V ai-first-fw/local-mcps/kibana/test_server.py  # 6 tests  — bsearch polling
```
