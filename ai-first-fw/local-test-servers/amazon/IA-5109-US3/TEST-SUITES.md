...
| `BODY-FULFILMENT-DATE` | a shipment carrying purchase instant `2026-08-20T09:12:03Z` | `FulfillmentDate` is a machine instant with timezone (accepting either 'Z' or explicit offset), ISO 8601 to the second, and NOT the buyer's purchase instant | `FulfillmentDate` on the received document; Amazon rates late shipment on this field | `D9` / `P-3` labelled interim proxy until `CR-3` lands; authority documents specify ISO 8601 dateTime with offset optional (13-F4:97, cross-border-fields §4.2), so spelling is unspecified between 'Z' and numeric offset | the notification carries no fulfilment instant at all — hence the proxy |
