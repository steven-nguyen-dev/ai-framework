...
`note`; the expectations themselves are in requirements.py with their citations.
...
The payload is built by transformer, a local stand-in for the JPluger Amazon
...
Usage:
  ...
  python3 amazon/IA-5105-US1/suite-taxonomy.py TAX-CAT-1 TAX-CAT-2       # only the cases named
  BASE=http://127.0.0.1:23103 BASE_OMS=http://127.0.0.1:23001 python3 amazon/IA-5105-US1/suite-taxonomy.py
...
class Checks:
    ...
    def add(self, label, what, expected, actual):
        ...
        self.items.append({"label": label, "what": what, "expected": self._repr_val(expected), "actual": self._repr_val(actual), "ok": ok})
...
#
# The producer is transformer, a local stand-in for the JPluger Amazon integration
# this harness cannot start. The expectations come from requirements, written from the
