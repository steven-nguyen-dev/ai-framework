#!/usr/bin/env python3
"""IA-5105-US1: End-to-End Multi-Marketplace Store Connect & Taxonomy Sync Suite (non-US).
...
every case names the document and section; requirements.py holds the expectations with their
...
WHAT IS UNDER TEST. The payload is built by transformer, a local stand-in for the
...
Publishes live status to amazon/test-results/IA-5105-US1-connect-non-us/run-<stamp>/results.json.

Usage:
  python3 amazon/IA-5105-US1/suite-connect-non-us.py
  python3 amazon/IA-5105-US1/suite-connect-non-us.py NONUS-PRE-1 NONUS-RBN-DE     # only the cases named
"""
...
# Suite moved one level down: keep its own folder (sibling suites, transformer,
# requirements) and amazon/ (shared fixture builders) both importable.
...
def publish():
    ...
    payload = {
        "suite": SUITE,
        "title": "IA-5105-US1: Amazon Multi-Marketplace Taxonomy Sync (non-US E2E)",
        "stamp": STAMP,
...
class Checks:
    ...
    def _repr_val(v):
        if isinstance(v, (list, dict)):
            s = f"<{type(v).__name__} with {len(v)} items>"
            if len(v) <= 5:
                raw = str(v)
                return raw if len(raw) <= 500 else s
            return s
        s = str(v)
        return (s[:500] + f"... ({len(s)} chars)") if len(s) > 500 else s
    ...
    def add(self, label, what, expected, actual):
        ...
        self.items.append({
            "label": label,
            "what": what,
            "expected": self._repr_val(expected),
            "actual": self._repr_val(actual),
            "ok": ok
        })
        return ok
...
#
# WHAT IS UNDER TEST. transformer is a local stand-in for the JPluger Amazon
# integration, which this harness cannot start. It builds the payload; the suite fires it at the OMS
# mock and judges what ARRIVED there. Expectations come from requirements, written from the
...
def _rbn_pair_checks(ch, calls, detail, store, expect_picker):
    ...
    Both expectations are computed by requirements.expected_rbn_rows from Amazon's own
    ...
    def _trim_field_values(d):
        if not isinstance(d, dict):
            return d
        out = dict(d)
        if "field_values" in out and isinstance(out["field_values"], list):
            vals = out["field_values"]
            if len(vals) > 5:
                out["field_values"] = vals[:3] + [{"name": f"... ({len(vals) - 3} more items)", "value": "..."}]
        return out

    if not child:
        detail[f"{label}_received_child"] = {}
        detail[f"{label}_expected_child"] = _trim_field_values(child_want)
    ...
    detail[f"{label}_received_child"] = _trim_field_values(child)
    detail[f"{label}_rows_in_posting"] = len(rows)
...
def c_nonus_picker_isolation(ch, calls, detail):
    ...
    detail["de_expected_picker_count"] = len(de_want)
    detail["de_expected_picker_sample"] = de_want[:3] if len(de_want) > 3 else de_want
    detail["es_expected_picker_count"] = len(es_want)
    detail["es_expected_picker_sample"] = es_want[:3] if len(es_want) > 3 else es_want
